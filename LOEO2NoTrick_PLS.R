...
#Dir <- "C:/Users/Marina/Documents/Berna/Practicas profesionales/Hyperparameter tuning/To Suecia"
Dir <- "/home/omontesinos/BCP/PLS"
setwd(Dir)
...
for(j in 1:length(Traits)){
  ...
  # LOEO CV Partition
  set.seed(2022)
  Envs=unique(Pheno$site)    #Unique Lines
  Envs
  length(Envs)
  folds <- cv_kfold(records_number = length(Envs), k = length(Envs))
  ...
  for (i in seq_along(folds)) {
    ...
    cat("\t\t\t\t*** Fold:", i, " ***\n")
    fold <- folds[[i]]
    Env_sam_i=Envs[fold$training]
    Env_sam_i
    fold_i <-which(Pheno$sit %in% Env_sam_i)
    length(fold_i);dim(X)[1]-length(fold_i);dim(X)[1]
    ...
    FoldPredictions <- data.frame(
      Fold = i,
      Line = Pheno$GID[-fold_i],
      Env = Pheno$site[-fold_i],
      Observed = y_testing,
  ...
  NewDir <- paste("LOEO2 CV", Traits[j], sep = "/")
  mkdir(NewDir)
...
for (i in seq_along(folds)) {
  ...
  #plot(y_testing[, c("BLUE_sugar")], predictions[, c("BLUE_sugar")])
